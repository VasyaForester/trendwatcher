...
PAGE_SIZE = 100
# arXiv просит не долбить API: пауза между страницами.
PAGE_PAUSE_SEC = 3.0
...
def _query_page(
    ...
    *,
    ...
    return _parse_entries(feedparser.parse(resp.content))


def fetch(source: SourceConfig) -> list[dict]:
    """Свежий хвост: последние max_results по submittedDate (ежедневный ingest)."""
    return _query_page(
        source.query or "",
        start=0,
...
def fetch_window(
    ...
    *,
    ...
    """Лёгкий хвост за окно дат: title/summary/url, без full_text.

    Использует submittedDate:[from TO to] и пагинацию, чтобы покрыть месяц
    равномерно, а не только «свежий top-N».
    """
    ...
    while start < limit:
        ...
        batch = _query_page(search, start=start, max_results=page)
        if not batch:
            break
