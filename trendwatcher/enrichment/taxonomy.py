...
TAXONOMY: dict[str, list[str]] = {
    ...
    # Побег модели/агента из sandbox/containment во время cyber-evals (OpenAI/HF, Anthropic, Kimi).
    "eval_containment_escape": [
        r"escaped? containment",
        r"(model|agents?|llm).{0,45}(escaped?|escape[sd]?|broke out).{0,35}(sandbox|containment)",
        r"(sandbox|containment) escape.{0,45}(model|agents?|llm|eval|ai|test)",
        r"(escaped?|escape[sd]?|broke out).{0,35}(sandbox|containment|test(?:ing)? environment).{0,45}(model|agents?|llm|ai|eval)",
        r"broke out of.{0,25}(sandbox|containment|test(?:ing)? environment|evaluation)",
        r"(cybersecurity|model|security) evaluation.{0,55}(incident|breach|unauthorized|compromis|internet)",
        r"(incident|breach|unauthorized|compromis).{0,55}(cybersecurity|model|security) evaluation",
        r"security incident during.{0,25}(model )?evaluation",
        r"evaluation (?:environment|sandbox).{0,45}(escape|breach|broke? out|internet|unauthorized)",
        r"rogue (?:agents?|models?).{0,40}(testing|eval|sandbox|containment|summer)",
        r"(model|agents?).{0,40}hack(?:ed|ing)?.{0,40}(during|while).{0,20}(testing|evaluation|eval)",
        r"(test|eval(?:uation)?|pre[- ]?release) (?:model|agent)s?.{0,35}escaped?",
        r"(model|agents?).{0,25}escaped?.{0,45}(broke into|hacked|compromis|unauthorized)",
    ],
...
FEED_EVENT_PATTERNS: list[str] = [
    ...
    r"escaped? containment",
    r"(sandbox|containment) escape",
    r"(model|agent).{0,40}(escaped?|broke out).{0,30}(sandbox|containment)",
    r"(cybersecurity|model) evaluation.{0,40}(incident|breach)",
    r"security incident during.{0,20}(model )?evaluation",
